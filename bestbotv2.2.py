...
class Player(BaseBot):
    def __init__(self) -> None:
        ...
        # Adaptive turn/river aggression with weak made hands (one pair / two pair).
        # Positive = past big turn/river aggression with weak made hands has worked.
        # Negative = it has been punished (we lost big pots); we should back off.
        self.turn_river_weak_aggr_score = 0
        self.hand_used_high_tr_weak_made = False

        # Adaptive preflop aggression with non-premium hands.
        # Uses an exponential moving average of success rate for aggressive
        # non-premium preflop plays vs this opponent, to adjust aggression smoothly.
        self.preflop_nonprem_aggr_ema = 0.5  # start neutral at 50% "success"
        self.hand_used_high_preflop_nonprem = False
    ...
    def on_hand_end(self, game_info: GameInfo, current_state: PokerState) -> None:
        ...
        # Adaptive update: if in this hand we used high turn/river aggression
        # with only one pair / two pair and saw the showdown result, update score.
        if self.hand_used_high_tr_weak_made:
            if current_state.payoff > 0:
                # Successful big aggression with weak made hand: small positive reinforcement.
                self.turn_river_weak_aggr_score += 1
            elif current_state.payoff < 0:
                # Failed big aggression with weak made hand: strong negative reinforcement.
                self.turn_river_weak_aggr_score -= 3
            # Keep within a reasonable band so it nudges rather than dominates.
            self.turn_river_weak_aggr_score = int(self._clip(self.turn_river_weak_aggr_score, -6, 6))

        # Adaptive update for aggressive non-premium preflop raises:
        # maintain an exponential moving average of success probability.
        if self.hand_used_high_preflop_nonprem:
            # Consider any hand where aggressive non-premium preflop was used and
            # we ended with a positive payoff as a "success", otherwise failure.
            outcome = 1.0 if current_state.payoff > 0 else 0.0
            alpha = 0.3  # weight of the most recent outcome
            self.preflop_nonprem_aggr_ema = (
                (1.0 - alpha) * self.preflop_nonprem_aggr_ema + alpha * outcome
            )
    ...
    def _detect_profiles(self, state: PokerState, pred_opp_bid: int) -> dict:
        ...
        # Detect "bestbot-like" heavy 4-bettor / preflop jammer profile.
        # These opponents:
        # - Frequently choose very large preflop raises (our opp_massive_pre_raise_hits high)
        # - And have a high overall preflop raise rate (opp_pfr_hits / opp_pfr_opps).
        opp_pfr_rate = self._rate(self.opp_pfr_hits, self.opp_pfr_opps, 0.0)
        heavy_4bettor = (
            self.opp_pre_raise_spots >= 6
            and self.opp_massive_pre_raise_hits >= 3
            and massive_pre_rate > 0.12
            and opp_pfr_rate > 0.28
        )

        # Value-heavy big bettor: rarely folds to our postflop bets, rarely stabs tiny,
        # and when making big bets they tend to have it (low huge-bet frequency overall
        # but high call/raise rate vs our bets). Against this profile we should be more
        # conservative with one-pair / two-pair bluff-catching on scary boards.
        value_heavy_big_bettor = (
            self.our_postflop_bet_spots >= 16
            and call_rate + raise_rate > 0.65
            and fold_rate < 0.30
            and huge_rate <= 0.30
        )
        ...
        return {
            ...
            "heavy_4bettor": heavy_4bettor,
            "value_heavy_big_bettor": value_heavy_big_bettor,
        }
    ...
    def _play_preflop(self, game_info: GameInfo, state: PokerState):
        ...
        # 3-bet more often with premiums versus very large opens so that we don't
        # just flat big raises with AA/KK/QQ and let bestbot realize equity cheaply.
        if (
            tier == 3
            and call_cost > 0
            and (call_cost > 400 or call_cost > 0.15 * eff_stack)
        ):
            raise_freq = min(1.0, raise_freq + 0.20)

        # Versus bestbot-like heavy 4-bettors, tighten our 3-bet range:
        # - In 3-bet spots (call_cost > 0), only raise with premium hands (tier 3).
        # - With tier 1–2, prefer calling or folding instead of 3-betting light.
        if profile.get("heavy_4bettor") and call_cost > 0 and tier <= 2:
            raise_freq *= 0.25
        ...
        # Adaptive preflop aggression with non-premium hands:
        # - Premiums (tier 3) are always allowed to be aggressive.
        # - For tiers 1–2, use an EMA-based success rate for aggressive
        #   non-premium preflop plays to smoothly adjust our raise frequency.
        if tier <= 2:
            # Map EMA in [0,1] to a multiplicative factor via a smooth exponential:
            # ema=0.5 -> factor ~1.0 (neutral)
            # ema low  -> factor <<1.0 (aggression damped)
            # ema high -> factor >>1.0 (more aggression allowed)
            ema = self.preflop_nonprem_aggr_ema
            k = 2.0  # sensitivity
            factor = math.exp(k * (ema - 0.5))
            # Clip factor so we never become completely passive or insane:
            factor = self._clip(factor, 0.3, 2.0)
            raise_freq *= factor
        ...
        if can_raise and roll < raise_freq:
            # Versus bestbot-like heavy 4-bettors, avoid expensive 3-bet/fold cycles:
            # if we are facing a sizable open (call_cost high) with a non-premium hand,
            # prefer just calling rather than reopening the betting and inviting a 4-bet.
            if profile.get("heavy_4bettor") and call_cost > 0 and tier <= 2:
                # If the price to call is already more than ~4BB (80), just flat.
                if call_cost > 80 and can_call:
                    return ActionCall()
            amount = self._choose_preflop_raise_size(state, tier, profile)
            # Mark aggressive non-premium preflop raises so we can update the score later.
            if tier <= 2:
                # Treat raises that take our total wager above a modest threshold as "aggressive".
                if amount > max(200, int(0.08 * state.my_chips)):
                    self.hand_used_high_preflop_nonprem = True
    ...
    def _play_postflop_facing_bet(
        ...
        has_big_made = self._has_big_made_hand(state.board, state.my_hand)
        ...
        if facing_reraise:
            ...
            if rev_danger > 0.40:
                call_thresh += 0.06
            # Don't stack off with one pair / two pair when they've shown strength (reraise).
            # If we don't have a big made hand, make calling and raising extremely selective
            # once a significant chunk of our stack is at risk.
            if not has_big_made:
                if stack_commit > 0.35 and adj_eq < 0.70:
                    call_thresh = 0.99
                if stack_commit > 0.25 and state.street in ("turn", "river"):
                    raise_thresh = 0.99
        ...
        if small_stab and state.can_act(ActionRaise) and adj_eq > small_stab_eq_min:
            ...
            if punish_gate and self.rng.random() < small_stab_punish_freq:
                ...
                if not has_big_made:
                    pressure = min(pressure, 0.40)
                amount = self._choose_postflop_raise_size(
                    state, adj_eq, pressure, texture, spr, state.street
                )
                self.hand_i_bet_postflop = True
                # Mark this as a high-aggression turn/river play with a weak made hand if applicable.
                if state.street in ("turn", "river") and not has_big_made:
                    self.hand_used_high_tr_weak_made = True
                return self._safe_raise_or_fallback(state, amount)
        ...
        # With only one pair / two pair on turn/river (no straight/flush/full house),
        # avoid putting in a very large fraction of our remaining stack just to call.
        if (
            state.street in ("turn", "river")
            and not has_big_made
            and stack_commit > 0.45
            and adj_eq < 0.70
            and state.can_act(ActionFold)
        ):
            return ActionFold()
        ...
        if state.street == "river":
            ...
            # 4) Versus value-heavy big bettors, be extremely conservative with
            # one-pair / two-pair bluff-catching on river.
            if (
                profile.get("value_heavy_big_bettor")
                and not has_big_made
                and (rel_price >= 0.40 or stack_commit > 0.35)
                and adj_eq < 0.78
                and state.can_act(ActionFold)
            ):
                return ActionFold()

            # 5) Correction 2: With only one pair / weak two-pair, fold on river only when
            # the bet is clearly large AND our hand is clearly weak. Medium bets (40–50% pot)
            # we call more with decent bluff-catchers (adj_eq 0.70+).
            if (
                not has_big_made
                and (rel_price >= 0.52 or stack_commit > 0.42)
                and adj_eq < 0.70
                and state.can_act(ActionFold)
            ):
                return ActionFold()
        ...
        if state.can_act(ActionRaise) and adj_eq > raise_thresh:
            # On river with only one pair / weak two-pair facing a big bet, do not raise –
            # treat as bluff-catcher or fold; never overbet for value with marginal hand.
            if state.street == "river" and not has_big_made and (rel_price >= 0.52 or stack_commit > 0.42):
                if state.can_act(ActionCall) and adj_eq >= call_thresh and rel_price < 0.55:
                    return ActionCall()
                if state.can_act(ActionFold):
                    return ActionFold()
                if state.can_act(ActionCall):
                    return ActionCall()
                return self._fallback_action(state)
            # On turn/river with only one pair or two pair, keep aggression modest:
            # do not fire big raises without a big made hand.
            if state.street in ("turn", "river") and not has_big_made:
                if stack_commit > 0.30:
                    # Too expensive to turn a marginal made hand into a bluff-catcher stack-off.
                    if state.can_act(ActionCall) and adj_eq >= call_thresh:
                        return ActionCall()
                    if state.can_act(ActionFold):
                        return ActionFold()
            ...
            # With only one pair / two pair, cap bet size to modest fraction of pot (≤ 40%).
            if not has_big_made:
                pressure = min(pressure, 0.40)
            amount = self._choose_postflop_raise_size(
                state, adj_eq, pressure, texture, spr, state.street
    ...
    def _play_postflop_checked_to(self, state, adj_eq, pot, spr, texture, profile, rev_danger):
        ...
        if is_calling_station:
            if adj_eq > value_thresh + 0.04:
                ...
                # With only a pair / two pair (no big made hand), keep bet sizes modest on all streets.
                if not has_big_made:
                    pressure = min(pressure, 0.40)
        ...
        if value_bet or bluff_ok or pressure_ok or semi_bluff_ok:
            ...
            # With only a pair / two pair, cap pressure so our bets aren't huge on any street.
            if not has_big_made:
                pressure = min(pressure, 0.40)
            amount = self._choose_postflop_raise_size(state, adj_eq, pressure, texture, spr, state.street)
            self.hand_i_bet_postflop = True
    ...
    def _choose_preflop_raise_size(self, state: PokerState, tier: int, profile: dict) -> int:
        ...
        # Versus bestbot-like heavy 4-bettors, shrink our 3-bet sizes with non-premium hands.
        # When facing a raise preflop (cost_to_call > 0) and this profile is detected,
        # cap our raise amount at a modest absolute size so that if they 4-bet/jam,
        # we have not already committed 35–40% of our stack and can fold cheaply.
        if profile.get("heavy_4bettor") and tier <= 2 and state.cost_to_call > 0:
            # Cap to roughly 15BB (300 chips) to keep 3-bets small with non-premiums.
            cap_abs = 300
            target = min(target, max(min_r, cap_abs))
    ...
    def _has_big_made_hand(self, board: list[str], my_hand: list[str]) -> bool:
        """
        Conservative detector for strong made hands on the board:
        returns True if we have at least a straight, flush, full house, or better,
        where our hole cards meaningfully participate. Used to distinguish big made
        hands from mere one pair / two pair when deciding how aggressively to play
        turn and river.
        """
        cards = board + my_hand
        if len(cards) < 5:
        ...
        # Check if we have a flush using at least one of our cards
        if flush_suit:
            my_flush_cards = [c for c in my_hand if c[1] == flush_suit]
            if my_flush_cards:
                return True

        # Rank counts for full house / quads
        ranks = [c[0] for c in cards]
        rank_counts = Counter(ranks)
        max_count = max(rank_counts.values())
        if max_count >= 4:
            # Four of a kind is always "big"
            return True
        ...
        # Straight detection: we require that the straight includes at least one of our hole-card ranks.
        unique_ranks = sorted({RANK_TO_INT[r] for r in ranks})
        # Wheel adjustment: treat A as 1 as well
        if 14 in unique_ranks:
            unique_ranks.append(1)
            unique_ranks = sorted(set(unique_ranks))

        def has_straight_with_hole() -> bool:
            hole_ranks = {RANK_TO_INT[c[0]] for c in my_hand}
            if 14 in hole_ranks:
                hole_ranks.add(1)
            seq_len = 1
            for i in range(1, len(unique_ranks)):
                if unique_ranks[i] == unique_ranks[i - 1] + 1:
                    ...
                    seq_len = 1
                if seq_len >= 5:
                    window = set(range(unique_ranks[i] - 4, unique_ranks[i] + 1))
                    if hole_ranks & window:
                        return True
            return False

        if has_straight_with_hole():
            return True
