...
class Player(BaseBot):
    ...
    def _play_preflop(self, game_info: GameInfo, state: PokerState):
        ...
        # Heads-up range expansion: against weak or small_raise_anchor opponents,
        # treat certain tier-0 hands as tier-1 (playable).
        # In heads-up, any Ace beats ~60%+ of random hands; K-x beats ~55%.
        # Folding these to a routine raise is surrendering significant EV.
        # We only upgrade when opp is detected weak OR has a fixed small raise pattern,
        # since those signals mean their raise doesn't represent a strong hand.
        if tier == 0:
            weak_or_anchor = (
            ...
            if weak_or_anchor and call_cost > 0:
                ...
                hi_rank = max(r1, r2)
                lo_rank = min(r1, r2)
                # Ax hands (any ace): upgrade to tier 1 against weak/fixed raisers
                # Kx (K2+), QJ, QT, JT off/suit: also worth calling small raises
                ...
                if (has_ace or (has_king and lo_rank >= RANK_TO_INT['5'])
                    ...
                    # Only upgrade if the raise is not massive (don't call 4-bets with A2)
                    if pot_odds <= 0.40:
                        tier = 1  # treat as playable for this decision
        ...
        # --- Exploit: versus massive 3/4-bets, flat with strong hands and
        #     widen calls with medium strength instead of re-raising.
        facing_massive_reraise = call_cost > 0 and call_cost > 0.22 * eff_stack
        if facing_massive_reraise:
            # Premiums (AA/KK/QQ/JJ/AK/AQ) – flat the huge raise instead of 4-betting again.
            is_premium_linear = hand_key in PREMIUM
            # Medium made hands like TT/99/88, AQ/KQ type holdings live in tier 2 here.
            is_medium_pair_or_broadway = tier == 2

            if is_premium_linear and state.can_act(ActionCall):
                return ActionCall()

            if is_medium_pair_or_broadway:
                # Versus IFailed's too-wide big 3/4-bets, call more often pre but avoid
                # committing entire stack with dominated hands.
                if state.can_act(ActionCall) and pot_odds <= 0.40:
                    return ActionCall()
                if state.can_act(ActionFold):
                    return ActionFold()
    ...
    def _play_postflop_facing_bet(
        ...
        # HOWEVER: vs weak opponents their bets are less credible even with info,
        # so apply a smaller penalty. We still want to muscle through weak folders.
        if self.opp_won_auction:
            ...
            # vs weak opp: halve the penalty (they bluff into us even with our card).
        ...
        if profile["massive_pre_jammer"]:
            call_thresh += 0.03
        if not have_info:
            # Without any revealed card info we're uncertain, so be more cautious.
            # vs weak opponents apply a smaller penalty since their bets are less reliable.
            ...
            # vs weak opp: smaller board penalty - they bet randomly on dangerous boards too.
        ...
        # Extra river all-in guards (only vs non-weak opponents):
        ...
        # vs weak opponents on river: still fold truly hopeless hands but allow thinner calls
