...
class Player(BaseBot):
    ...
    def _detect_profiles(self, state: PokerState, pred_opp_bid: int) -> dict:
        ...
        if self.opp_bid_exact_samples:
            ...
            for x in self.opp_bid_exact_samples:
                ...
                if 8 <= x <= 12:
                    band += 1
                # Known fixed bids (incl. 125 which opponents often use)
                if x in (125, 398, 597, 1245):
                    high_fixed += 1
            tiny_rate = tiny / total
            band_rate = band / total
            high_fixed_rate = high_fixed / total
            # Detect any repeated bid (e.g. always 125): mode frequency
            try:
                mode_count = Counter(self.opp_bid_exact_samples).most_common(1)[0][1]
                mode_rate = mode_count / total
                high_fixed_rate = max(high_fixed_rate, mode_rate)
            except (IndexError, ZeroDivisionError):
                pass
            mean_bid = sum(self.opp_bid_exact_samples) / total
            fixed_moderate_bidder = high_fixed_rate > 0.28 and mean_bid < 250
        else:
            fixed_moderate_bidder = False
        ...
        return {
            ...
            "fixed_moderate_bidder": fixed_moderate_bidder,
    ...
    def _play_postflop_facing_bet(
        ...
        if facing_reraise:
            ...
            if stack_commit > 0.25 and adj_eq < 0.70:
                raise_thresh = 0.99
            if stack_commit > 0.30 and adj_eq < 0.68:
                call_thresh = 0.99
        ...
        # Don't go broke on one pair: avoid raising when equity is marginal and stack already committed
        if stack_commit > 0.32 and adj_eq < 0.68:
            raise_thresh = max(raise_thresh, 0.99)
        ...
        if state.street == "river":
            ...
            # 2) Don't call large river overbets with marginal one pair / second pair / bottom pair
            if rel_price >= 0.50 and adj_eq < 0.55 and state.can_act(ActionFold):
                return ActionFold()
            if rel_price >= 0.40 and adj_eq < 0.58 and state.can_act(ActionFold):
                return ActionFold()
    ...
    def _choose_auction_bid(self, game_info: GameInfo, state: PokerState, t0: float) -> int:
        ...
        elif profile.get("fixed_moderate_bidder"):
            # Opponent bids one fixed amount (e.g. 125) often: outbid when we have value
            if eq_without >= 0.42 and state.my_chips > pred_opp_bid + 5:
                target = pred_opp_bid + (2 + int(self.rng.uniform(0, 6)))
                if value_of_info > 0.02:
                    target += int(0.08 * pot)
            else:
                target = max(0, pred_opp_bid - 5) if self.rng.random() < 0.35 else pred_opp_bid + 1
