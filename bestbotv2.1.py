...
class Player(BaseBot):
    ...
    def _play_postflop_facing_bet(
        ...
        if state.street == "river":
            ...
            # 5) Correction 2: With only one pair / weak two-pair, never put in a large
            # fraction of stack on river when facing a significant bet. Fold or call small only.
            if (
                not has_big_made
                and (rel_price >= 0.40 or stack_commit > 0.35)
                and adj_eq < 0.78
        ...
        if state.can_act(ActionRaise) and adj_eq > raise_thresh:
            ...
            if state.street == "river" and not has_big_made and (rel_price >= 0.40 or stack_commit > 0.35):
