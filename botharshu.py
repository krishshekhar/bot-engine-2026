...
class Player(BaseBot):
    ...
    def _play_preflop(self, game_info: GameInfo, state: PokerState):
        ...
        if tier == 3:
            ...
            raise_freq, call_freq = 0.62, 0.22
        else:
            raise_freq, call_freq = 0.28, 0.12
    ...
    def _play_postflop(self, game_info: GameInfo, state: PokerState, t0: float):
        ...
        if call_cost > 0:
            return self._play_postflop_facing_bet(state, adj_eq, pot, call_cost, pot_odds, spr, texture, profile, rev_danger)

        return self._play_postflop_checked_to(state, adj_eq, pot, spr, texture, profile, rev_danger)

    def _play_postflop_facing_bet(self, state, adj_eq, pot, call_cost, pot_odds, spr, texture, profile, rev_danger):
        ...
        if rel_price > 0.60:
            call_thresh = max(call_thresh, 0.42)
        if rel_price > 0.80:
            call_thresh = max(call_thresh, 0.48)
    ...
    def _play_postflop_checked_to(self, state, adj_eq, pot, spr, texture, profile, rev_danger):
        ...
        value_thresh = 0.56 if state.street == "flop" else 0.58
    ...
    def _choose_preflop_raise_size(self, state: PokerState, tier: int, profile: dict) -> int:
        ...
        target = min_r + int(span * self._clip(frac, 0.06, 0.92))
    ...
    def _should_call_preflop_allin(self, state: PokerState, tier: int, profile: dict) -> bool:
        ...
        if high >= 12 and low >= 10:
            return True
        if suited and abs(r1 - r2) <= 2 and low >= 7:
            return True
